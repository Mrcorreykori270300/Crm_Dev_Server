...
namespace Inwinteck_CRM.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            try
            {
                    ...
                    switch (result)
                    {
                        case SignInStatus.Success:
                            ...
                            if (user.Status == 1)
                            {
                                ...
                                {
                                    string msg = "Login" + " " + User.Identity.GetUserName();
                                    utlity.createlog(msg);
                                    return RedirectToAction("IndexAdmin", "Dashboard");
                                }
                                else if (UserManager.IsInRole(SignInManager.AuthenticationManager.AuthenticationResponseGrant.Identity.GetUserId(), "Help Desk Manager"))
                                {
                                    string msg = "Login" + " " + User.Identity.GetUserName();
                                    utlity.createlog(msg);
                                    return RedirectToAction("IndexHelpDesk", "Dashboard");
                                }
                                else if (UserManager.IsInRole(SignInManager.AuthenticationManager.AuthenticationResponseGrant.Identity.GetUserId(), "Sr.Help Desk Manager"))
                                {
                                    string msg = "Login" + " " + User.Identity.GetUserName();
                                    utlity.createlog(msg);
                                    return RedirectToAction("IndexSrHelpDesk", "Dashboard");
                                }
                                else if (UserManager.IsInRole(SignInManager.AuthenticationManager.AuthenticationResponseGrant.Identity.GetUserId(), "Field Engineer"))
                                {
                                    string msg = "Login" + " " + User.Identity.GetUserName();
                                    utlity.createlog(msg);
                                    ...
                                    else
                                    {
                                        ...
                                        return RedirectToAction("FEProfile", "Master", new { id = fe });
                            ...
                            {
                                string msg = "Login" + " " + User.Identity.GetUserName() + " " + "Your status is deactive, Contact admin.";
                                utlity.createlog(msg);
                ...
                else if (logintype == "HRMS")
                {
                    ...
                    if (cnt > 0)
                    {
                        ...
                        return RedirectToAction("Index", "HRMS", new { emp = empuser.Emp_Id });
        ...
        public ActionResult ChangePassword(string Password)
        {
            ...
            {
                TempData["message"] = "Error Occured while Saving!!";
        ...
        public ActionResult Inwin_Engg(FE_Master_Personal sa, string Others, string Storage, string Networking, string Server, string Laptop, string Desktop)
        {
            try
            {
                if (this.IsCaptchaValid("Captcha is not valid"))
                {
                    ...
                    //FE_Master_Skill fems = new FE_Master_Skill();
                    //fems.FE_ID = sa.Id;
                    //fems.Laptop = Laptop;
                    //fems.Desktop = Desktop;
                    //fems.Storage = Storage;
                    //fems.Others = Others;
                    //fems.Networking = Networking;
                    //fems.Server = Server;
                    //fems.CreatedBy = "";
                    //fems.CreatedOn = DateTime.Now;
                    //db.FE_Master_Skill.Add(fems);
                    //db.SaveChanges();
